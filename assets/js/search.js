...
document.addEventListener('DOMContentLoaded', function() {
    var searchIndexURL = window.searchIndexURL || '/index.json';
    ...
    function renderResults(docsToShow) {
        ...
        docsToShow.forEach(function(doc) {
            var dateHtml = doc.dateFormatted ?
                '<div class="post-meta"><time datetime="' + escapeHtml(doc.date) + '">' +
                escapeHtml(decodeHtmlEntities(doc.dateFormatted)) + '</time></div>' : '';
            html += '<article class="post-item">';
            html += '<h3><a href="' + escapeHtml(doc.url) + '">' +
                    escapeHtml(decodeHtmlEntities(doc.title)) + '</a></h3>';
            html += dateHtml;
            html += '<p class="post-summary">' +
                    escapeHtml(truncateSummary(doc.summary || doc.content || '', 350)) + '</p>';
            html += '</article>';
        });
        html += '</div>';
    ...
    function runSearch() {
        ...
        if (docsToShow.length === 0) {
            searchResults.innerHTML = '<div class="search-results"><p class="search-no-results">Your search for "<strong>' +
                                    escapeHtml(query) + '</strong>" did not return any results.</p></div>';
            return;
        }
